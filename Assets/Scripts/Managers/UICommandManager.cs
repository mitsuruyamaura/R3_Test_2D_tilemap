...
public class UICommandManager : MonoBehaviour {
    ...
    public void SetupUICommandManager () {
    ...
    private void SetDefaultGroupAndDetail() {
        ...
        // TODO 選択可能なグループか判定
        // 行動済、移動済の場合、該当のグループの各ボタンは非アクティブにする
    ...
    public void ExecuteCommandProc(CommandGroupType groupType) {
        //gameManager.ChangeState(CommandStateType.CursorMove);


        // 届いた CommandGroupType によって分岐
        switch (groupType) {
            case CommandGroupType.Move:
                gameManager.ChangeState(CommandStateType.UnitMove);
                break;
            case CommandGroupType.Action:
                gameManager.ChangeState(CommandStateType.TargetSelect);
                break;
        ...
        // 全ボタンを一旦移動にする
        //gameManager.ChangeState(CommandStateType.UnitMove);
        Debug.Log($"ExecuteCommandProc : {gameManager.CommandState.Value}");
    }


    public void ReturnCursorMove() {
        gameManager.ChangeState(CommandStateType.CursorMove);
    }
}